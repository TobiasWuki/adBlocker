...
namespace utils{
    ...
    /*  anscheindend Überfüssig
    namespace toEndians { 

        static uint16_t toLittleEndian(uint16_t networkValue) {   

            uint16_t high = networkValue >> 8; //high value aus sicht des Inputs vom Netz

            uint16_t low = networkValue << 8; //low value aus sicht des Inputs vom Netz

            uint16_t pcVersion = high | low;
        ...
        static uint16_t toBigEndian(uint16_t pcValue) {
            ...
            uint16_t low = pcValue >> 8; //low value aus sicht des Inputs vom PC

            uint16_t high = pcValue << 8; //high value aus sicht des Inputs vom PC

            uint16_t netVersion = high | low;
    ...
    */
    ...
    //    std::string upstreamDNS = "1.1.1.1";


    //    static int receiveQuery(char* queryBuffer, sockaddr_in ipClient, int outPort53Socket, int maxSize) {
    ...
    //    static bool send(char* gaslightMsg, sockaddr_in ipClient, int outPort53Socket, int byteCount) {
    ...
    //    static bool forward(char* queryBuffer, std::string UPSTREAM_DNS, int byteCount, int& outInternetSocket) {
    ...
    //    static int receiveAnswer(char* queryBuffer, int& outInternetSocket, int maxSize) {
    ...
    //    static void gaslightAppointeeStan(char* queryBuffer) {
    ...
    //    static void initSockets(int& outPort53Socket, int& outInternetSocket) {
